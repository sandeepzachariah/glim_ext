...
public:
  ...
  GNSSGlobal() : logger(create_module_logger("gnss_global")) {
    ...
    logger->info("gnss_topic={}", gnss_topic);
  ...
  void gnss_callback(const NavSatFixConstPtr& gnss_msg) {
    logger->info("GNSS data received: {} {}", gnss_msg->header.stamp.sec, gnss_msg->header.stamp.nanosec);
  ...
  void gnss_callback(const NavSatFixConstPtr& msg) {
    ...
    logger->info("GNSS data received: {} {}", msg->header.stamp.sec, msg->header.stamp.nanosec);
    // Convert lat/lon/alt to UTM or local ENU
    data.position = latlon_to_utm(msg->latitude, msg->longitude, msg->altitude);
  
    // Extract 3x3 position covariance matrix (row-major order)
    for (int i = 0; i < 3; ++i)
      for (int j = 0; j < 3; ++j)
